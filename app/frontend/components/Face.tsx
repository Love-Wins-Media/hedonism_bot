...
const faceImageVariants = cva(
  "w-8 h-8 rounded-full overflow-hidden shrink-0 ring-1",
  {
    variants: {
      selected: {
...
const faceButtonVariants = cva(
  "flex items-center gap-2.5 px-2 py-1.5 rounded-sm transition-colors text-left",
  {
    variants: {
      selected: {
...
export default function Face({ face, onSelect, selected }: FaceProps) {
  ...
  return (
    <button
      className={faceButtonVariants({ selected: selected })}
      onClick={() => {
        if (onSelect) {
          onSelect(data.id);
        }
